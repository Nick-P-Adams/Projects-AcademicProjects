public class MinHeap 
{
	...
	public MinHeap(int maxSize)
	{
		...
		this.currentSize = 0;
		this.heap = new LinkedList[this.maxSize];
	}
	
	public void insert(LinkedList vertex)
	{
		...
		this.heap[this.currentSize] = vertex;
			
		int current = this.currentSize;
			
		while(this.heap[current].getWeight() < this.heap[this.parent(current)].getWeight())
		{
	...
	{
		LinkedList removed = this.heap[0];
	...
	private void minHeapify(int i)
	{
		...
		if(!(this.isLeaf(i)))
		{
			if(this.heap[i].getWeight() > this.heap[left].getWeight() || this.heap[i].getWeight() > this.heap[right].getWeight())
			{
